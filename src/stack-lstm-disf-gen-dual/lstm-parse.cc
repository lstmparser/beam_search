...
#include "cnn/lstm.h"
#include "logging.h"
#include "stack-lstm-disf-gen-dual/c2.h"
...
double evaluate(const po::variables_map &conf, ParserBuilder &parser, const std::string &dev_result,
    ...
    for (unsigned sii = 0; sii < dev_size; ++sii) {
        ...
        vector<unsigned> pred = parser.log_prob_parser(&hg1, current_gen_feature, sentenceUnkStr, sentence, tsentence,
                                                       sentencePos,
                                                       vector<unsigned>(),
    ...
    if (conf.count("train")) {
        ...
        for (unsigned iter = 0; iter < maxiter; ++iter) {
            ...
            for (unsigned i = 0; i < order.size(); ++i) {
                ...
                {
                    ComputationGraph hg;
                    parser.log_prob_parser(&hg, current_gen_feature, sentenceUnkStr, sentence, tsentence, sentencePos,
                                           actions,
                                           corpus.actions,
                                           corpus.intToWords,
                                           false, &right);
